...
/****** Object:  StoredProcedure [dbo].[usp_OrderBulkOperations]    Script Date: 3/17/2020 1:33:13 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[usp_OrderBulkOperations]
...
--BEGIN TRANSACTION -- Remove after testing
...
BEGIN
    SET @SQL = 'UPDATE tblSegment ' +
               '   SET ' + @FieldName + ' = ' + CASE WHEN @SearchValue IS NULL OR RTRIM(@SearchValue) = ''  THEN '''' + REPLACE(@ReplaceValue,'''','''''') + '''' ELSE 'REPLACE(' + @FieldName + ',''' + REPLACE(@SearchValue,'''','''''') + ''',''' + REPLACE(@ReplaceValue,'''','''''') + ''')' END + 
...
BEGIN
    ...
    SELECT @MaxGroupNo = MAX(iGroupNumber)
      FROM tblSegmentSelection a
     WHERE SegmentID IN (SELECT CAST(value as int) SegmentID FROM STRING_SPLIT(@TargetSegments, ',')) 
       AND SegmentID <> @SourceSegment

    INSERT INTO tblSegmentSelection
            (SegmentID,cQuestionFieldName,cQuestionDescription,cJoinOperator,iGroupNumber,iGroupOrder,cGrouping,cValues,cValueMode,cDescriptions,cValueOperator,cFileName,cSystemFileName,cTableName,cValueDescription, dCreatedDate,cCreatedBy)
    SELECT b.SegmentID,cQuestionFieldName,cQuestionDescription,cJoinOperator,@MaxGroupNo + iGroupNumber,iGroupOrder,cGrouping,cValues,cValueMode,cDescriptions,cValueOperator,cFileName,cSystemFileName,cTableName,cValueDescription, GETDATE(),@UserID
      FROM tblSegmentSelection a
     CROSS JOIN (SELECT CAST(value as int) SegmentID FROM STRING_SPLIT(@TargetSegments, ',')) b
     WHERE a.SegmentID = @SourceSegment
