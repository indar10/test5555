...
using Infogroup.IDMS.Test.Base.TestData;
...
namespace Infogroup.IDMS.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="IDMSDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(IDMSDbContext context)
            {
        ...
        protected void UsingDbContext(Action<IDMSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<IDMSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<IDMSDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<IDMSDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }
        

        protected void UsingDbContext(int? tenantId, Action<IDMSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IDMSDbContext>())
                {
        ...
        //protected async Task UsingDbContextWithDatabaseConnectionString(Action<IDMSDbContext> action)
        //{
        //    IDMSDbContext _context;
        ...
        //    builder.UseSqlServer($"Server=10.50.104.12; Database=DW_Admin; Trusted_Connection=False;User ID=WebUser;Password=Infogroup@001;")
        //            .UseInternalServiceProvider(serviceProvider);

        //    _context = new IDMSDbContext(builder.Options);
        //    _context.Database.Migrate();
        //     action(_context);
        //    await _context.SaveChangesAsync();
        //}

        protected async Task UsingDbContextAsync(int? tenantId, Func<IDMSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IDMSDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<IDMSDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<IDMSDbContext, Task<TResult>> func)
        {
            TResult result;
