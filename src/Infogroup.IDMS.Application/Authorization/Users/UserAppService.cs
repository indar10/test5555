...
using Infogroup.IDMS.Authorization.Users.Exporting;
...
{
    [AbpAuthorize(AppPermissions.Pages_Administration_Users, AppPermissions.Pages_FastCount_History)]
    public class UserAppService : IDMSAppServiceBase, IUserAppService
    {
        ...
        public UserAppService(
            ...
            IRepository<IDMSUser> userRepository,
        ...
        public async Task<FileDto> GetUsersToExcel(GetUsersToExcelInput input)
        {
            ...
            var userListDtos = ObjectMapper.Map<List<UserListDto>>(users);
            await FillRoleNames(userListDtos);
            var updatedUserList = new List<UserListDto>();

            foreach (var user in users)
            {
                var newUser = userListDtos.FirstOrDefault(x => x.Id == user.Id);                
                var grantedPermissions = await UserManager.GetGrantedPermissionsAsync(user);

                var permissionNames = grantedPermissions.Where(x => !x.Name.Equals("Pages")).OrderBy(x => x.Name).ToList();
                ...
                permissionList.Sort();
                newUser.DivisionCode = GetDivisionCode(newUser.UserName);
                if (newUser != null)
                {
                    foreach (var item in permissionList)
                    {
                        var updatedUser = new UserListDto
                        {
                            ...
                            GrantedPermissions = item,
                            DivisionCode = newUser.DivisionCode
                        };
                        updatedUserList.Add(updatedUser);
                    }
                }
            }
            return _userListExcelExporter.ExportToFile(updatedUserList);
        }
        ...
        public async Task UnlockUser(EntityDto<long> input)
        {
            var user = await UserManager.GetUserByIdAsync(input.Id);
            user.Unlock();
            // Insert row in user login attempt table
            var loginAttempts = _userLoginAttempt.GetAll().Where(x => x.UserId == input.Id).OrderByDescending(x => x.Id).FirstOrDefault();
            var userLoginAttempts = new UserLoginAttempt
            {
                BrowserInfo = loginAttempts.BrowserInfo,
                ClientIpAddress = loginAttempts.ClientIpAddress,
                CreationTime = DateTime.Now,
                Result = AbpLoginResultType.Success,
                TenancyName = loginAttempts.TenancyName,
                TenantId = loginAttempts.TenantId,
                UserId = loginAttempts.UserId,
                UserNameOrEmailAddress = loginAttempts.UserNameOrEmailAddress
            };
            await _userLoginAttempt.InsertAsync(userLoginAttempts);
        ...
        protected virtual async Task UpdateUserAsync(CreateOrUpdateUserInput input)
        {
            ...
            else if (!input.User.Password.IsNullOrEmpty())
            {
                ...
                _passwordManager.CheckRecentPasswords(user, input.User.Password);
        ...
        public void InsertUserLoginAttempt(UserLoginAttempt objUserLoginAttempt)
        {
            var userLoginAttempt = ObjectMapper.Map<UserLoginAttempt>(objUserLoginAttempt);
            _userLoginAttempt.Insert(userLoginAttempt);
            CurrentUnitOfWork.SaveChanges();
        }

        private string GetDivisionCode(string userName)
        {
            var query = (from abpUser in _abpUserRepository.GetAll()
                         join tblUser in _userRepository.GetAll() on abpUser.UserName.ToLower() equals tblUser.cUserID.ToLower()
                         join tblLookUp in _lookupRepository.GetAll() on tblUser.LK_AccountingDivisionCode equals tblLookUp.cCode
                         where (tblUser.cUserID.ToLower().Equals(userName.ToLower()) && tblLookUp.cLookupValue.Equals("AccountingDivisionCode"))
                         select tblLookUp.cDescription).FirstOrDefault();

            return string.IsNullOrEmpty(query)? "" : query.ToString();
        }
    }
}
