...
(function (root, factory) {
    ...
    var QueryBuilder = function ($el, options) {
         ...
         * @readonly
         */
        this.unMappedFilters = this.settings.unMappedFilters;
        /**
         * List of icons
    ...
    QueryBuilder.selectors = {
        group_container: '.rules-groups-container',
        ...
        textbox_rule: '[data-textbox=rule]',
        select_rule: '[data-select=rule]',
        file_rule: '[data-file=rule]',
        ...
        filters.forEach(function (filter, i) {
            ...
            switch (filter.input) {
                ...
                case 'select':
                    ...
                    Utils.iterateOptions(JSON.parse(filter.values), function (value, label, optgroup) {
                        cleanValues.push({
                            value: value,
                            label: label,
    ...
    QueryBuilder.prototype.bindEvents = function () {
        ...
        this.$el.on('click.queryBuilder', Selectors.add_rule, function () {
            var $group = $(this).closest(Selectors.group_container);
            // 1524 - Code for opening filter dropdown
        ...
        this.$el.on('click.queryBuilder', Selectors.textbox_rule, function () {
            var $rule = $(this).closest(Selectors.rule_container);
            self.toRuleTextbox(self.getModel($rule));
        });
        this.$el.on('click.queryBuilder', Selectors.select_rule, function () {
            var $rule = $(this).closest(Selectors.rule_container);
            self.toRuleSelect(self.getModel($rule));
        });
        this.$el.on('click.queryBuilder', Selectors.file_rule, function () {
            var $rule = $(this).closest(Selectors.rule_container);
            self.forFileUpload(self.getModel($rule));
        });
    ...
    QueryBuilder.prototype.deleteGroup = function (group) {
        ...
        var deletedGroupPos = parseInt(group.id.split("group_")[1]);
        if (deletedGroupPos < group.parent.rules.length) {
            group.parent.rules.forEach((val) => {
                if (deletedGroupPos > 0) {
                    deletedGroupPos--;
                    return;
                }
    ...
    QueryBuilder.prototype.updateGroupCondition = function (group, previousCondition) {
        ...
        if (group.id.includes("_group_0") && group.rules.length > 0) {
            group.rules.forEach((val) => {
                val.rules[0].isRuleUpdated = val.rules[0].isRuleUpdated === 0 ? val.rules[0].isRuleUpdated : 1;
            });
        } else if (group.rules.length > 1) {
            var flag = 1;
            group.rules.forEach((val) => {
                if (flag !== 1) {
                    val.isRuleUpdated = val.isRuleUpdated === 0 ? val.isRuleUpdated : 1;
                }
                flag++;
            });
        }
     ...
     */
    QueryBuilder.prototype.addRule = function (parent, data, flags, ruleId, mapUnMapped) {
        /**
        ...
        this.createRuleFilters(model, ruleId, mapUnMapped);

        // 1524 - Code for opening filter dropdown
        var isOpen = (typeof data !== 'undefined') && data.openDropdown;
        if (isOpen) {
            var targetName = model.id + '_filter';
            var target = document.getElementsByName(targetName);
            $(target).trigger('chosen:open');
            event.stopPropagation();
            this.focusDropdown = target;
        }
    ...
    QueryBuilder.prototype.deleteRule = function (rule) {
        ...
        if (rule.parent.rules.length > 1 && rule.selectionId === rule.parent.rules[0].selectionId) {
            rule.parent.rules[1].isRuleUpdated = rule.parent.rules[1].isRuleUpdated === 0 ? rule.parent.rules[1].isRuleUpdated : 1;
        }

        if (rule.selectionId > 1) {
            this.deletedRules.push(rule.selectionId);
        }
    ...
    /**
     * Tries to change input to textbox
     * @param {Rule} rule
     * @returns {boolean} if the rule has been deleted
     */
    QueryBuilder.prototype.toRuleTextbox = function (rule) {
        rule.cModeType = "T";
        rule.isDirectFileUpload = 0;
        rule.isRuleUpdated = rule.isRuleUpdated === 0 ? rule.isRuleUpdated : 1;
        if (rule.operator.op === "Between" || rule.operator.op === "Not Between") {
            rule.operator.nb_inputs = 2;
        }
        this.clearErrors(rule);
     ...
     * @returns {boolean} if the rule has been deleted
     */
    QueryBuilder.prototype.toRuleSelect = function (rule) {
        rule.cModeType = "G";
        rule.isDirectFileUpload = 0;
        rule.isRuleUpdated = rule.isRuleUpdated === 0 ? rule.isRuleUpdated : 1;
        if (rule.operator.op === "Between" || rule.operator.op === "Not Between") {
            rule.operator.nb_inputs = 1;
        }
        this.clearErrors(rule);
        this.createRuleInput(rule);
        if (rule.value !== undefined) {
    ...
    QueryBuilder.prototype.getCookieByName = function (name) {
        var v = document.cookie.match('(^|;) ?' + name + '=([^;]*)(;|$)');
        return v ? v[2] : null;
    };

    QueryBuilder.prototype.displayError = function (rule, alert) {
        var e = this.trigger('validationError', rule, alert, '');
        if (!e.isDefaultPrevented()) {
            rule.error = alert;
        }
    };
     ...
     * @returns {boolean} if the rule has been deleted
     */
    QueryBuilder.prototype.forFileUpload = function (rule) {
        var upperThis = this;
        
        var campaignId = this.campaignId;
        var baseUrl = this.baseUrl;
        if (rule.operator.op !== "In" && rule.operator.op !== "Not In") {
            rule.operator = upperThis.operators[0];
        }
        rule.value = '';
        rule.cModeType = "F";
        rule.isDirectFileUpload = 0;
        rule.cSystemFileName = '';
        rule.isRuleUpdated = rule.isRuleUpdated === 0 ? rule.isRuleUpdated : 1;
        var inputType = rule.filter.input;
        rule.filter.input = "textFile";
        this.createRuleInput(rule);
        var input = $("#" + rule.$el[0].id).children(".rule-value-container").children(".form-control");
        input.splice(-1, 1);
        input.attr('type', 'file');
        rule.filter.input = inputType;
        var authToken = this.getCookieByName("Abp.AuthToken");
 
        input.fileupload({
           
            url: baseUrl + '/File/UploadFile?campaignId=' + campaignId,
            beforeSend: function (xhr, data) { 
                upperThis.trigger('triggerFileUploadBlock', true);
            ...
            success: function (response, status) {
                upperThis.clearErrors(rule);
                var ruleId = rule.$el[0].id;
                var ruleValueContainer = $("#" + rule.$el[0].id).children(".rule-value-container");
                rule.filter.input = "textarea";
                upperThis.createRuleInput(rule);
                var newInput = ruleValueContainer.children("textarea");

                newInput.val(response.result.substring(19));
                rule.cFileName = response.result;                
                rule.value = response.result.substring(19);
                rule.filter.input = inputType === "select" ? "select" : "textarea";
                upperThis.trigger('triggerFileUploadBlock', false);
            },
            error: function (error) {
                // Error callback 
                upperThis.trigger('triggerFileUploadBlock', false);
                rule.value = undefined;
                var alert = new Array(error.responseJSON.result.value);
                upperThis.displayError(rule, alert);
            }
     ...
     * @private
     */
    QueryBuilder.prototype.createRuleFilters = function (rule, ruleId, mapUnMapped) {
        /**
        ...
        var clonedFilters = [];
        var isUnMappedFieldExisted = typeof this !== 'undefined' && typeof this.unMappedFilters !== 'undefined' && this.unMappedFilters.length > 0;

        if (mapUnMapped && isUnMappedFieldExisted) {
            clonedFilters = $.merge([], filters);
            var unmappedFilter = $.grep(this.unMappedFilters, function (rawField) {
                return rawField.id == ruleId
            });
            if (unmappedFilter.length > 0 && typeof unmappedFilter[0] !== 'undefined') {
                var newfilter = $.extend(filters[0], unmappedFilter[0])
                clonedFilters.push(newfilter);
            }
        } else {
            clonedFilters = filters
        }
        var $filterSelect = $(this.getRuleFilterSelect(rule, clonedFilters));
     ...
     * @param {Rule} ruleafterCreateRuleInput
     * @fires QueryBuilder.
     * @private
     */
    ...
    QueryBuilder.prototype.createRuleInput = function (rule) {
        ...
        if (preSavedValue !== undefined) {
            rule.value = preSavedValue;
        }
    ...
    QueryBuilder.prototype.updateRuleFilter = function (rule, previousFilter) {
        ...
        // Resetting cModeType when filter changed 
        ...
        // for ishowdefault
        if (rule.cModeType === 1 || rule.cModeType === null || rule.cModeType === undefined) {
            if (rule.filter.data.iShowDefault === 3 && rule.filter.data.iFileOperations === true) {
                this.forFileUpload(rule);
            }
        }
    };
    ...
    QueryBuilder.prototype.updateRuleOperator = function (rule, previousOperator) {
        ...
        if (!rule.operator || rule.operator.nb_inputs === 0) {
            ...
            if (rule.isRuleUpdated > 1) {
                if (typeof previousOperator !== 'undefined' && !(previousOperator.op === rule.operator.op)) {
                    rule.isRuleUpdated = typeof previousOperator === 'object' && previousOperator.length === 0 ? rule.isRuleUpdated : 1;
                }
            }

            //Added so that for between operator only one rule input can be created
            if ((rule.operator.op === "Between" || rule.operator.op === "Not Between") && (rule.filter.input === "checkbox" || rule.filter.input === "select")) {
                if (previousOperator.op === "Between" || previousOperator.op === "Not Between") {
                    rule.operator.nb_inputs = 2;
                    if (rule.cModeType === "G" || rule.cModeType === 1) {
                        rule.operator.nb_inputs = 1;
                    }
                } else if (rule.cModeType === "T") {
                    rule.operator.nb_inputs = 2;
                    rule.value = '';
                } else {
                    rule.operator.nb_inputs = 1;
                }
            } else if (rule.operator.op === "Between" || rule.operator.op === "Not Between") {
                rule.operator.nb_inputs = 2;
                if (previousOperator.op !== "Between" && previousOperator.op !== "Not Between") {
                    if (rule.value !== undefined) {
                        rule.value = rule.value.length > 1 ? [] : "";
                ...
                if ((previousOperator.op === "Between" || previousOperator.op === "Not Between") && ((rule.filter.input !== "checkbox" && rule.filter.input !== "select") || rule.cModeType === "T")) {
                    if (rule.value !== undefined) {
                        rule.value = rule.value.length > 1 ? [] : "";
                    }
                }
    ...
    QueryBuilder.prototype.updateRuleValue = function (rule, previousValue) {
        ...
        if (rule.isRuleUpdated > 1) {
            if (rule.cModeType === 'G') {
                if (typeof previousValue !== "undefined" && (previousValue.length !== undefined || Object.keys(previousValue).length > 0) && typeof previousValue === "object") {
                    rule.isRuleUpdated = rule.value === null || rule.value.length !== Object.keys(previousValue).length ? 1 : rule.isRuleUpdated;
                }
            } else if (typeof previousValue !== "undefined" && (rule.operator.op === "Between" || rule.operator.op === "Not Between")) {
                rule.isRuleUpdated = previousValue[0] === rule.value[0] && previousValue[1] === rule.value[1] ? rule.isRuleUpdated : 1;
            } else if (typeof previousValue !== "undefined" && previousValue !== rule.value) {
                rule.isRuleUpdated = 1;
            }
        }
    ...
    QueryBuilder.prototype.validate = function (options) {
        ...
        var valid = (
            function parse(group) {
                var done = 0;
                var errors = 0;
                // code to skip validation when user saves and all rules and groups are deleted except root group
                if (group.level === 1 && group.rules.length === 0 && self.deletedRules.length > 0) {
                    return true;
                }
    ...
    QueryBuilder.prototype.checkEmptyGroup = function () {
        if (this.model.root.rules.length === 0) {
            return -1;
        } else if (this.model.root.rules.length === 1 && this.model.root.rules[0].rules.length === 0) {
            return -1;
        } else if (this.model.root.rules.length === 1 && this.model.root.rules[0].rules.length === 1 &&
            this.model.root.rules[0].rules[0].filter === null) {
            return -1;
        }
        return 0;
    };

    QueryBuilder.prototype.getSegmentSelectionRules = function (skipValidation, options) {
        options = $.extend({
            get_flags: false,
            allow_invalid: false,
            skip_empty: false
        ...
        if (skipValidation) {
            var valSkip = this.checkEmptyGroup();
            if (valSkip === -1 && this.deletedRules.length === 0) {
                list.push(valSkip);
                return list;
            }
        }
        ...
        (function parse(group) {
            ...
            if (parseInt((group.id).split("group_")[1]) === 0) {
                headerCondition = group.condition;
            }

            group.each(function (rule) {
                if (rule.cModeType === "T" || rule.cModeType === 1) {
                    var field = rule.filter.field.toUpperCase();
                    if (field.indexOf('ZIPRADIUS') > -1)
                        isValidate = upperThis.validateZipRadius(rule);
                    else if ((field.indexOf('GEOMAPPING') > -1)) 
                        isValidate = upperThis.validateGeoMapping(rule);
                    else if ((field.indexOf('GEORADIUS') > -1))
                        isValidate = upperThis.validateGeoRadius(rule);
                    if (isValidate === false) {
                        isError++;
                        return null;
                    }
                ...
                // If file input from DB is F type then this will run
                if (rule.cModeType === "F") {
                    cTemp = rule.filter.input;
                    rule.filter.input = "file";
                }

                // If file input from DB is G type and is not checkbox then this will run
                if (rule.cModeType === "G" && rule.filter.input !== "checkbox") {
                    rule.filter.input = "select";
                }

                // If file input from DB is T type then this will run
                if (rule.cModeType === "T") {
                    cTemp = rule.filter.input;
                    rule.filter.input = "text";
                }

                if (rule.filter.field === "ZipRadius") {
                    cMode = rule.cModeType === 1 ? "T" : rule.cModeType;
                    cDescriptions = "";
                    cValues = value;
                    cFileName = rule.cFileName === null ? value : rule.cFileName;
                    rule.cSystemFileName = cMode === "T" ? "" : rule.cSystemFileName;
                    rule.filter.input = cTemp;
                } else {
                    switch (rule.filter.input) {
                        case 'select':
                        case 'checkbox':
                            cMode = "G";
                            if (value instanceof Array) {
                                rule.cSystemFileName = "";
                                cDescriptions = value.map(a => a.split('|||')[1]).toString();
                                cValues = value.map(a => a.split('|||')[0]).toString();
                                if (rule.operator.op === "Between" || rule.operator.op === "Not Between") {
                                    cValues = cValues.replace(",", "-");
                                }

                            } else {
                                cDescriptions = value.split('|||')[1];
                                cValues = value.split('|||')[0];
                                rule.cSystemFileName = "";
                            }
                            break;
                        case 'file':
                        ...
                        default:
                            ...
                            cDescriptions = "";
                            rule.cSystemFileName = "";
                            cValues = rule.operator.op === "Between" || rule.operator.op === "Not Between" ? $.trim(rule.value[0]) + '-' + $.trim(rule.value[1]) : $.trim(value);
                            if (cTemp === "select") {
                                rule.filter.input = cTemp;
                            }
                ...
                rule.cModeType = cMode;
                isValidate = upperThis.validateOperators(rule);
                if (isValidate === false) {
                    isError++;
                    return null;
                }

                //Adding Segment Selection details in the list
                var segmentSelectionRuleData = {

                    id: rule.selectionId,
                    cQuestionFieldName: rule.filter.field === "ZipRadius" ? "ZIP" : (rule.filter ? rule.filter.field : null),
                    cQuestionDescription: rule.filter.field === "ZipRadius" ? "Zip Radius" : "",
                    cJoinOperator: group.condition,
                    cValueOperator: rule.operator.type,
                    cValues: cValues,
                    cValueMode: cMode,
                    iGroupNumber: parseInt(((group.id).split("group_")[1])) + 1,
                    iGroupOrder: 1,
                    cFileName: cMode === 'F' ? cFileName : "",
                    cTableName: rule.filter.data.cTableName,
                    cDescriptions: cDescriptions,
                    cSystemFileName: rule.cSystemFileName,
                    isRuleUpdated: rule.isRuleUpdated,
                    isDirectFileUpload: rule.isDirectFileUpload,
                    cGrouping: rule.cGrouping,
                    cCreatedBy: rule.cCreatedBy,
                    dCreatedDate: rule.dCreatedDate
                };

                list.push(segmentSelectionRuleData);
            }, function (model) {
                var data = parse(model);
        ...
        var countByGroupNumber = count(list, function (item) {
            return item.iGroupNumber;

        });
        list.push(countByGroupNumber);
        list.push(headerCondition);
        list.push(this.deletedRules);
        list.push(isError);
        return list;
    };

    QueryBuilder.prototype.validateZipRadius = function (rule) {

        var upperThis = this;
        var isValidate = true;

        if (rule.operator.op !== "In" && rule.operator.op !== "Not In") {
            upperThis.displayError(rule, new Array("invalid_operator"));
            return false;
        }

        var re = /^[a-zA-Z0-9]{5,6}([\-]\d{1,4}(\.\d{1,2})?)+(\s{0,}\,\s{0,}[a-zA-Z0-9]{5,6}([\-]\d{1,4}(\.\d{1,2})?))*$/;
        var values = rule.value.includes(",") ? rule.value.split(",") : rule.value.split("\n");
        var reg = /^\d{1,4}(\.\d{1,2})?$/;
        var isGlobalRedious = false;
        var val = "";
        for (var i = 0; i < values.length; i++) {
            val = values[i];
            if (values[0].indexOf("radius") === 0 && values[i].indexOf("radius") === 0) {
                isGlobalRedious = true;
                //match redius string with int
                val = val.replace("radius", "").replace("=", "");
                val = $.trim(val);
                if (!reg.test(val)) {
                    upperThis.displayError(rule, new Array("invalid_radius_format_decimal"));
                    isValidate = false;
                    break;
                }
                continue;
            }
            if (!isGlobalRedious && !re.test(values[i])) {
                upperThis.displayError(rule, new Array("invalid_radius_format"));
                isValidate = false;
                break;
            } else if (isGlobalRedious && !(values[i].length === 5 && reg.test(values[i]))) {
                upperThis.displayError(rule, new Array("invalid_zip_format"));
                isValidate = false;
                break;
            }
        }
    ...
    QueryBuilder.prototype.validateGeoRadius = function (rule) {
        var enteredValue = rule.value.toLowerCase();
        enteredValue = $.trim(enteredValue).replace(/\n/g, ",");

        if (enteredValue[enteredValue.length - 1] == ',') {
            enteredValue = enteredValue.slice(0, enteredValue.length - 1);
        }
        if (rule.operator.op !== "In" && rule.operator.op !== "Not In") {
            this.triggerValidationError(rule, "invalid_operator", null);
            return false;
        }

        var values = enteredValue.split(',');
        var re = /-?\d+\.\d+\|-?\d+\.\d+\:\d{1,4}(\.\d{1,2})?/g;
        var iBlanks = 0;
        var lineNumber = 0;
        var errors = [];
        for (var i = 0; i < values.length; i++) {
            ...
            if (values[i].match(re) != values[i] || values[i].indexOf('|') < 0) {
                lineNumber = i + 1 - iBlanks;
                errors = ["invalid_geo_radius_latlong", lineNumber]
                this.triggerValidationError(rule, errors, null);
                return false;
            }
            if (i > 4) {
                this.triggerValidationError(rule, "invalid_geo_radius_maxlimit", null);
                return false;
            }
        }
    ...
    QueryBuilder.prototype.validateGeoPolygon = function (rule) {
        var enteredValue = rule.value.toLowerCase();
        enteredValue = $.trim(enteredValue).replace(/\n/g, "|");
        ...
        var re = /-?\d+\.\d+:-?\d+\.\d+/g;
        for (var i = 0; i < values.length; i++) {
            if (values[i].match(re) != values[i]) {
                errors.push("invalid_geo_polygon_format");
                this.triggerValidationError(rule, errors, null);
                return false;
            }
        }

        values = values.filter(function (currentValue) {
            return currentValue;
        });

        if (values.length < 3) {
            errors.push("invalid_geo_polygon_paths");
            this.triggerValidationError(rule, errors, null);
            return false;
        } 
        if (values[0] != values[values.length - 1]) {
            errors.push("invalid_geo_polygon_incomplete");
            this.triggerValidationError(rule, errors, null);
            return false;
        }

        if (hasDuplicates(values.slice(0, -1))) {
            errors.push("duplicate_geo_polygon_path");
            this.triggerValidationError(rule, errors, null);
            return false;
        }
    ...
    QueryBuilder.prototype.validateGeoMapping = function (rule) {
        var enteredValue = JSON.parse(rule.value);
        var newRule = jQuery.extend(true, {}, rule);
        newRule.value = enteredValue.VALUE;
        if (enteredValue.TYPE === "CIRCLE") {
            return this.validateGeoRadius(newRule);
        } else {
            return this.validateGeoPolygon(newRule);
        }      
    };
    ...
    QueryBuilder.prototype.validateOperators = function (rule) {
        var upperThis = this;
        ...
        if (rule.cModeType === "T") {
            ...
            if ((rule.operator.op === "In" || rule.operator.op === "Not In") && rule.filter.field !== "ZipRadius") {
                if (rule.filter.field === "GeoMapping") {
                    enteredValue = JSON.parse(enteredValue).VALUE.trim();
                }
                var parsedValuesArray = enteredValue.split('-');

                if (parsedValuesArray && parsedValuesArray.length === 2 &&
                    parsedValuesArray[0].trim().length > 0 &&
                    parsedValuesArray[1].trim().length > 0) {
                    if (enteredValue.search(',') === -1) {
                        upperThis.displayError(rule, new Array("invalid_operator_in"));
                        return false;
                    } else {
                        var parsedValuesCommaArray = enteredValue.split(',');
                        if (parsedValuesCommaArray) {
                            if (parsedValuesCommaArray[0].trim().length === 0 || parsedValuesCommaArray[1].trim().length === 0) {
                                upperThis.displayError(rule, new Array("invalid_operator_in"));
                                return false;
                            }
                        }
            ...
            if (rule.operator.op === "Between" || rule.operator.op === "Not Between") {
                var isNewLine = enteredValue[0].search('\n') > -1;
                var isNewLineOne = enteredValue[1].search('\n') > -1;
                if (isNewLine || isNewLineOne) {
                    upperThis.displayError(rule, new Array("invalid_new_line"));
                    return false;
                }

                if (enteredValue[0].trim().match(regex) !== null || enteredValue[1].trim().match(regex) !== null) {
                    upperThis.displayError(rule, new Array("invalid_character"));
                    return false;
                }

                if (enteredValue[0].includes(',') || enteredValue[1].includes(',')) {
                    upperThis.displayError(rule, new Array("invalid_comma_between"));
                    return false;
                }
            } else {
                if ((rule.operator.op === "Like" || rule.operator.op === "Not Like") && enteredValue.trim().match(regexOne) !== null) {
                    upperThis.displayError(rule, new Array("invalid_character"));
                    return false;
                }
                if (rule.operator.op !== "Like" && rule.operator.op !== "Not Like" && enteredValue.trim().match(regex) !== null) {
                    upperThis.displayError(rule, new Array("invalid_character"));
                    return false;
                }                     
            }

            if (rule.operator.op === "Less Than" || rule.operator.op === "Greater Than") {
                if (rule.filter.data.cFieldType !== null && rule.filter.data.cFieldType === "M") {
                    upperThis.displayError(rule, new Array("invalid_operator"));
                    return false;
        ...
        if (rule.cModeType === "G") {
            if (rule.operator.op === "Less Than" || rule.operator.op === "Greater Than") {
                if (enteredValue.length > 1) {
                    upperThis.displayError(rule, new Array("invalid_number_of_value"));
                    return false;
                }
            ...
            if (rule.operator.op === "Between" || rule.operator.op === "Not Between") {
                upperThis.displayError(rule, new Array("invalid_operator_between"));
                return false;
            }
        }
        return true;
    ...
    QueryBuilder.prototype.setRules = function (data, options) {
        ...
        $(this.focusDropdown).trigger('chosen:open');
        event.stopPropagation();

        if (!data || !data.rules || (data.rules.length === 0 && !this.settings.allow_empty)) {
            // Commented out because we do not want to throw error when rule returned from DB is empty
        ...
        (function add(data, group) {
            ...
            data.rules.forEach(function (item) {
                ...
                if (item.rules !== undefined) {
                    ...
                    var mapUnMapped = item.iIsListSpecific && item.iIsRAWNotMapped && typeof item.id !== 'undefined';

                    model = self.addRule(group, item.data, self.parseRuleFlags(item), item.id, mapUnMapped);
                    if (model === null) {
                        return;
                    }

                    // Copying the cValueMode coming from DB to model  
                    if (item.field === "ZIP" && item.value.includes('.')) {
                        var val = item.value.toLowerCase();
                        if (val.includes('.txt') || val.includes('.csv') || val.includes('.zip')) {
                            model.cModeType = item.cValueMode;
                        }
                    ...
                    model.cFileName = item.cFileName;
                    model.cSystemFileName = item.cSystemFileName;
                    model.selectionId = item.selectionId;
                    model.cGrouping = item.cGrouping;
                    model.cCreatedBy = item.cCreatedBy;
                    model.dCreatedDate = item.dCreatedDate;

                    if (!item.empty && item.id !== 0) {
                        // Now directly passing false as value instead of !options.allow_invalid 
                        // so that no error is thrown when field is non-selectable
                        var tempFilter = self.getFilterById(item.id, false);
                        if (tempFilter !== null) {
                            model.filter = tempFilter;
                        }
                    }
                    ...
                    if (model.operator && model.operator.nb_inputs !== 0) {
                        if (item.value !== undefined) {
                            if ((model.operator.op === "Between" || model.operator.op === "Not Between") && model.cModeType === "G") {
                                model.value = item.value.replace("-", ",");

                            } else if ((model.operator.op === "Between" || model.operator.op === "Not Between") && model.cModeType === "T") {
                                var temparr = [];
                                temparr[0] = item.value.split("-")[0];
                                temparr[1] = item.value.split("-")[1];
                                model.value = temparr;
                            } else {
                                if ((item.value.includes("\\") || item.cFileName.includes("\\")) && model.cModeType === "F") {
                                    var lastIndex = item.value.lastIndexOf("\\");
                                    model.value = item.value.substring(lastIndex + 1);
                                    model.isDirectFileUpload = 1;
                                }
                                else if ((item.value.includes("/") || item.cFileName.includes("/")) && model.cModeType === "F") {
                                    var lastIndex = item.value.lastIndexOf("/");
                                    model.value = item.value.substring(lastIndex + 1);
                                    model.isDirectFileUpload = 1;
                                }
                    ...
                    model.isRuleUpdated = item.selectionId;
    ...
    QueryBuilder.prototype.getFilterById = function (id, doThrow) {
        ...
        var isUnMappedFieldExisted = typeof this !== 'undefined' && typeof this.unMappedFilters !== 'undefined' && this.unMappedFilters.length > 0;
        if (isUnMappedFieldExisted) {
            for (var i = 0, l = this.unMappedFilters.length; i < l; i++) {
                if (this.unMappedFilters[i].id == id) {
                    return this.unMappedFilters[i];
                }
            }
        }
    ...
    QueryBuilder.prototype.getRuleInputValue = function (rule) {
        ...
        if (filter.valueGetter) {
            ...
            for (var i = 0; i < operator.nb_inputs; i++) {
                ...
                var name1 = Utils.escapeElementId(rule.id + '_value_' + (i+1));
                var tmp;

                switch (filter.input) {

                    case 'checkbox':
                        if (rule.cModeType === "T" || rule.cModeType === "F") {
                            ...
                            if (Object.keys(filter.values).length === 1) {
                                if (tmp.length > 0)
                                    tmp = [];
                                tmp.push($value.find('[name=' + name + ']').val());
                                $value.find('[name=' + name + ']').prop('checked', true);
                            }
                            value.push(tmp);
                        }
                        break;
                    ...
                    default:
                        var fileName = $value.find('[name=' + name1 + ']').val();
                        var authToken = this.getCookieByName("Abp.AuthToken");
                        if (fileName !== '' && fileName !== undefined && rule.operator.nb_inputs < 2)
                        {
                            if ($value.find('[name=' + name + ']').val() !== '')
                                $(idSuppress).val("");
                            if ($(idSuppress).val() !== '') {
                                var upperThis = this;
                                $.ajax({
                                    url: this.baseUrl + '/File/ValidateSuppressionFile',
                                    ...
                                    success: function (response) {
                                        ...
                                        upperThis.createRuleInput(rule);
                                        if (rule.filter.field !== "ZipRadius") {
                                            rule.cSystemFileName = response.result;
                                            rule.cFileName = response.result;
                                        }
                                                                                
                                        rule.isDirectFileUpload = 1;                                        
                                       
                                        rule.value = fileName.trim();
                                        var newInput = ruleValueContainer.children("text");
                                        newInput.val(fileName.trim());
                                    },
                                    error: function (error) {
                                        rule.value = undefined;      
                                        var alert = new Array(error.responseJSON.result.value);
...
<div id="{{= it.group_id }}" class="rules-groups-container"> \
  <div class="rules-group-header"> \
    <div class="pull-right group-actions"> \
   {{? it.level>1 }} \
    <button type="button" class="btn btn-xs btn-success-addrule" data-add="rule"> \
        <i class="{{= it.icons.add_rule }}"></i> {{= it.translate("add_rule") }} \
      </button> \
      {{?}} \
      {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }} \
        <button type="button" class="btn btn-xs btn-success ml-0" data-add="group"> \
          <i class="{{= it.icons.add_group }}"></i> {{= it.translate("add_group") }} \
        </button> \
      {{?}} \
      {{? it.level>1 }} \
        <button type="button" class="btn btn-xs btn-danger ml-0" data-delete="group"> \
  ...
  <div class=rules-group-body> \
    ...
    <div class="rules-group-header" style="padding-top: 5px;"> \
     <div class="pull-right group-actions"> \
       {{? it.level === 1}} \
         <button type="button" class="btn btn-xs btn-success ml-0" data-add="group"> \
           <i class="{{= it.icons.add_group }}"></i> {{= it.translate("add_group") }} \
...
<div id="{{= it.rule_id }}" class="rule-container row no-gutters"> \
  <div class="rule-header justify-content-center order-last align-self-center"> \
  <div class="btn-group pull-right rule-actions ml-1"> \
      <button type="button" class="btn btn-xs btn-danger" data-delete="rule"> \
        <i class="{{= it.icons.remove_rule }}"></i>\
      </button> \
    </div> \
	<div class="btn-group pull-right rule-actions btnFile ml-1" data-file="rule"    style="display:none"> \
      <button \type="button" class="btn btn-xs btn-primary" title="Upload a text file with one value per line"> \
        <i class="fa fa-upload"></i>\
      </button> \
    </div> \
	<div class="btn-group pull-right rule-actions btnSelect ml-1" data-select="rule"   style="display:none"> \
      <button type="button" class="btn btn-xs btn-primary" title="Select Values"> \
      <i class="far fa-caret-square-down"></i>\
      </button> \
    </div> \
    <div class="btn-group pull-right rule-actions btnTextbox ml-1" data-textbox="rule" style="display:none"> \
      <button type="button" class="btn btn-xs btn-primary" title="Enter Values separated by comma or one per line"> \
        <i class="fa fa-edit"></i>\
      </button> \
    </div> \
  </div> \
  {{? it.settings.display_errors }} \
    <div class="error-container align-self-center mr-2"><i class="{{= it.icons.error }}"></i></div> \
  {{?}} \
  <div class="rule-filter-container align-self-center mr-2"></div> \
  <div class="rule-operator-container align-self-center mr-2"></div> \
  <div class="rule-value-container col align-self-center"></div> \
</div>';
...
<select class="form-control"  name="{{= it.rule.id }}_filter"> \
  ...
  {{~ it.filters: filter }} \
    {{? optgroup !== filter.optgroup }} \
      ...
      {{?}} \
    {{?}} \
    <option {{? filter.data.iIsListSpecific === true }}class="group-option-color-red"{{?}} value="{{= filter.id }}" {{? filter.icon}}data-icon="{{= filter.icon}}"{{?}}>{{= it.translate(filter.label) }}</option> \
  {{~}} \
  {{? optgroup !== null }}</optgroup>{{?}} \
</select>';
...
<select class="form-control  {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
  {{~ it.operators: operator }} \
    {{? optgroup !== operator.optgroup }} \
      ...
      {{?}} \
    {{?}} \
    <option value="{{= operator.type }}" {{? operator.icon}}data-icon="{{= operator.icon}}"{{?}}>{{= it.translate("operators", operator.op) }}</option> \
  {{~}} \
  {{? optgroup !== null }}</optgroup>{{?}} \
</select>';
...
<select class="form-control" name="{{= it.name }}" {{? it.rule.filter.multiple }}multiple{{?}}> \
  ...
  {{? optgroup !== null }}</optgroup>{{?}} \
    ...
    QueryBuilder.prototype.getRuleInput = function (rule, value_id) {
        ...
        var name1 = rule.id + '_value_' + (++value_id);
        var c = filter.vertical ? ' class=block' : '';
        var h = '';
        var cTemp = '';

        // Temporarily changing default filter.input to "file when cValueMode is F.
        if (rule.cModeType === "F" && rule.filter.input !== "text" && rule.filter.input !== "textFile") {
            
            cTemp = filter.input;
            filter.input = "file";
        }

        // When user clicks on text button on a filter which supports dropdown or checkbox
        if (rule.cModeType === "T" && (rule.filter.input === "select" || rule.filter.input === "checkbox")) {
            cTemp = filter.input;
        ...
        if (typeof filter.input == 'function') {
            ...
            switch (filter.input) {
                ...
                case 'file':
                    h += '<textarea class="form-control" style="width:100%" name="' + name + '" disabled';
                ...
                case 'textarea':
                    h += '<textarea class="form-control" style="width:100%" name="' + name + '"';
                    if (filter.size) h += ' cols="' + filter.size + '"';
                    if (filter.rows) h += ' rows="' + filter.rows + '"';
                    if (validation.min !== undefined) h += ' minlength="' + validation.min + '"';
                    if (validation.max !== undefined) h += ' maxlength="' + validation.max + '"';
                    ...
                    // Restoring input type when input type is select
                    if (cTemp === "select" || cTemp === "checkbox") {
                        filter.input = cTemp;
                    }
                    break;
                ...
                case 'number':
                    ...
                    break;
                case 'textFile':
                    h += '<input class="form-control" type="text" style="width:50%" name="' + name + '"';
                    if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
                    if (filter.type === 'string' && validation.min !== undefined) h += ' minlength="' + validation.min + '"';
                    if (filter.type === 'string' && validation.max !== undefined) h += ' maxlength="' + validation.max + '"';
                    if (filter.size) h += ' size="' + filter.size + '"';
                    h += '>'
                    h += '<label style="width:5%"> - </label>';
                    h += '<input id="idSuppress" class="form-control" type="text" title="Please enter file name" style="width:45%" name="' + name1 + '"';
                    if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
                    if (filter.type === 'string' && validation.min !== undefined) h += ' minlength="' + validation.min + '"';
                    if (filter.type === 'string' && validation.max !== undefined) h += ' maxlength="' + validation.max + '"';
                    if (filter.size) h += ' size="' + filter.size + '"';
                ...
                default:
                    h += '<input class="form-control" type="text" style="width:100%" name="' + name + '"';
                    if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
                    if (filter.type === 'string' && validation.min !== undefined) h += ' minlength="' + validation.min + '"';
                    if (filter.type === 'string' && validation.max !== undefined) h += ' maxlength="' + validation.max + '"';
                    if (filter.size) h += ' size="' + filter.size + '"';
    ...
    Utils.groupSort = function (items, key) {
        ...
        items.forEach(function (item) {
            ...
            if (item[key] === "favourite" && optgroups.length > 0 && optgroups[0] === "allFilters") {
                optgroups.splice(0, 0, item[key]);
    ...
    var Node = function (parent, $el) {
        ...
        this.__.cFileName = null;

        this.__.cSystemFileName = null;

        this.__.selectionId = 0;

        this.__.isRuleUpdated = 0;

        this.__.isDirectFileUpload = 0;

        this.__.cGrouping = null;

        this.__.cCreatedBy = null;

        this.__.dCreatedDate = null;
    ...
    Utils.defineModelProperties(Node, ['level', 'error', 'data', 'flags', 'cModeType', 'cFileName', 'cSystemFileName', 'selectionId', 'isRuleUpdated', 'cGrouping', 'cCreatedBy', 'dCreatedDate', 'isDirectFileUpload']);
  ...
  <label for="' + id + '">' + val + '</label> \
    ...
    QueryBuilder.define('chosen-selectpicker', function (options) {
        ...
        this.on('aftercreaterulefilters', function (e, rule) {
            rule.$el.find(Selectors.rule_filter).children('form-control').css('width', '269px');
            rule.$el.find(selectors.rule_filter).removeclass('form-control').chosen(options);
        });

        this.on('afterCreateRuleOperators', function (e, rule) {
            rule.$el.find(Selectors.rule_operator).children('form-control').css('width', '180px');
    ...
    QueryBuilder.define('chosen-selectpicker', function (options) {
        ...
        this.on('triggerFileUploadBlock', function (e) { });
    ...
    QueryBuilder.define('sortable', function (options) {
                ...
                interact(node.$el[0])
                    .draggable({
                        ...
                        onend: function (event) {
                            ...
                            if (event.dropzone) {
                                src.parent.parent.rules.forEach((val) => {
                                    val.rules.forEach((item) => {
                                        item.isRuleUpdated = item.isRuleUpdated === 0 ? item.isRuleUpdated : 1;
                                    });
                                });
        ...
        if (!options.disable_template) {
            ...
            //    if (level > 1) {
            //        var $h = $(h.value);
            //        $h.find(QueryBuilder.selectors.condition_container).after('<div class="drag-handle align-self-center"><i class="' + options.icon + '"></i></div>');
            //        h.value = $h.prop('outerHTML');
            //    }
            //});
            ...
            this.on('getRuleTemplate.filter', function (h) {
                var $h = $(h.value);
                $h.find(QueryBuilder.selectors.rule_header).after('<div class="drag-handle align-self-center"><i class="' + options.icon + '"></i></div>');
                h.value = $h.prop('outerHTML');
            });
        }
    }, {
            inherit_no_sortable: true,
            inherit_no_drop: false,
    ...
    QueryBuilder.extend( /** @lends module:plugins.SqlSupport.prototype */ {
        ...
        getSQLtblsegmentSelection: function (skipValidation) {
            var segmentSelectionList = this.getSegmentSelectionRules(skipValidation);
            if (segmentSelectionList != null)
                return segmentSelectionList;
            else
                return null;
        },
         ...
         */
        setRulesFromSQL: function (query, campaignid, baseUrl, stmt) {

            var rulesObj = JSON.parse(query);
            this.deletedRules = [];
            this.campaignId = campaignid;
            this.baseUrl = baseUrl;
            this.setRules(rulesObj);
        },
    ...
    QueryBuilder.regional["en"] = {
      ...
      errors: {
        ...
        file_exceed_max: "File size is over the limit. Max limit is 1 GB.",
        "missing_CR+LF":
          "The last line of file is missing carriage return + line feed (ENTER key).",
        invalid_file_type: "Only txt, csv or zip files can be uploaded.",
        invalid_file_typeS3: "Only txt, csv or gz files can be uploaded.",
        invalid_file_name: "Please enter a valid file name",
        invalid_zip_extension: "ZIP file is not allowed for Zip Radius.",
        file_not_exist: "File does not exist",
        invalid_radius_format_decimal:
          "Enter Radius keyword in Radius=XX format,where XX is a decimal number up to 2 decimal places and less than 10000.",
        invalid_radius_format:
          "Enter radius value in XXXXXX-Radius format for US & Canadian Zip/Postal Codes, where radius is a decimal number up to 2 decimal places and less than 10000.",
        invalid_zip_format: "Entered zip data is not in correct format",
        invalid_new_line: "No new line in text",
        invalid_comma: "Comma is not allowed with LIKE operation",
        invalid_number_of_value: "Only one value allowed",
        invalid_character: "Invalid Character",
        invalid_operator_in:
          "Please select the Between or Not Between operator for format: Value-Value",
        invalid_number_of_input: "Select exactly two values",
        invalid_operator_between:
          "BETWEEN/NOT BETWEEN operator can not be used with list box or checkbox.",
        invalid_operator: "Invalid operator. Please select IN/NOT IN operator.",
        invalid_comma_between: "Comma is not allowed with Between operator",
        invalid_geo_radius_latlong:
          "Georadius should be in XX.X|XX.X:RR format,where RR is a decimal number up to 2 decimal places and less than 10000.Failed at Record No: {0}.",
        invalid_geo_radius_maxlimit: "Maximum 5 Geo Radius values are allowed.",
        invalid_filename_quotes: "Single quote is not allowed in file name.",
        invalid_geo_polygon_paths: "Geopolygon should have atleast 3 paths.",
        invalid_geo_polygon_incomplete:
          "First and last paths of geopolygon should be same.",
        invalid_geo_polygon_format:
          "Geopolygon paths should be in XX.XX:XX.XX format",
        duplicate_geo_polygon_path: "Geopolygon path should not have duplicate values except for first and last path.",
      },
      invert: "Invert",
      NOT: "NOT",
    };
