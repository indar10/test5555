...
using Infogroup.IDMS.Web.Helpers;
using System.IO;
...
{
    [AbpMvcAuthorize]
    public class TenantCustomizationController : IDMSControllerBase
    {
        ...
        public async Task<ActionResult> GetTenantLogo(string skin, int? tenantId)
        {
            ...
            using (CurrentUnitOfWork.SetTenantId(tenantId.Value))
            {
                ...
                if (skin.Equals("dark"))                
                    path = "assets\\common\\images\\data_axle_logo_dark_background.png";                
                else
                    path = "assets\\common\\images\\data_axle_logo_light_background.png";
                path = Path.Combine(webRootPath, path);
                byte[] imageByteData = System.IO.File.ReadAllBytes(path);
                return File(imageByteData, MimeTypeNames.ImagePng);

                // Uncomment if want to upload and get file by DB
                //return File(logoObject.Bytes, tenant.LogoFileType);
            }
        }
