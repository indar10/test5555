...
using Infogroup.IDMS.Web.Authentication.JwtBearer;
using Infogroup.IDMS.Web.Authentication.TwoFactor;
using Infogroup.IDMS.Web.Models.TokenAuth;
using Infogroup.IDMS.Authorization.Impersonation;
...
using Infogroup.IDMS.Web.Authentication.External;
...
namespace Infogroup.IDMS.Web.Controllers
{
    ...
    public class TokenAuthController : IDMSControllerBase
    {
        ...
        private readonly IIDMSJwtSecurityStampHandler _securityStampHandler;
        ...
        public TokenAuthController(
            ...
            IIDMSJwtSecurityStampHandler securityStampHandler,
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            // Temporary adding check to skip account lock error for stage.idms
            if (!Request.Headers["origin"].ToString().Contains("stage.idms")) {
                var lockOrWarning = _passwordManager.GetLastPasswordChange(model.UserNameOrEmailAddress);
                if (!string.IsNullOrEmpty(lockOrWarning))
                {
                    if (lockOrWarning.Contains("expire"))
                        warningMessage = lockOrWarning;
                    else
                        throw new UserFriendlyException(lockOrWarning);
                }

                // Do not allow user to login if the last logon date was over X number of days.
                // Get userloginattempt table and config items for the validation
                var totalDaysFromLastLogin = _userManager.GetUserPasswordRenewalDate(model.UserNameOrEmailAddress);
                var renewalDays = Convert.ToInt32(_lookupCache.GetLookUpFields("EXPSPAN").FirstOrDefault().cDescription);
                if (totalDaysFromLastLogin > renewalDays)
                {
                    // Insert a row in AbpUserLoginAttempts where resulttype = 9(locked out)
                    var userloginAttempt = new UserLoginAttempt
                    {
                        BrowserInfo = _logInManager.ClientInfoProvider.BrowserInfo,
                        ClientIpAddress = _logInManager.ClientInfoProvider.ClientIpAddress,
                        CreationTime = DateTime.Now,
                        Result = AbpLoginResultType.LockedOut,
                        TenancyName = GetTenancyNameOrNull(),
                        TenantId = 1,
                        UserId = _userManager.userLoginAttempt(model.UserNameOrEmailAddress).UserId,
                        UserNameOrEmailAddress = model.UserNameOrEmailAddress
                    };
                    //_userAppService.InsertUserLoginAttempt(userloginAttempt);
                    _userManager.InsertUserLoginAttempt(userloginAttempt);

                    //update AbpUser tables lastlogondateutc
                    _userManager.UpdateUser(model.UserNameOrEmailAddress);
                    // throw user firendly message
                    throw new UserFriendlyException(L("AccountLockedAdmin"));
                }
            }
            ...
            var key = $"{AppConsts.DefaultDatabaseId}_{loginResult.User.Id}";
            var cache= _redisCacheHelper.GetString(key);
            if (cache != null)
                _redisCacheHelper.KeyDeleteWithPrefix(key);
        ...
        {
            return _settingManager.GetSettingValue<bool>(AppSettings.HostManagement.AllowOneConcurrentLoginPerUser);
        }
