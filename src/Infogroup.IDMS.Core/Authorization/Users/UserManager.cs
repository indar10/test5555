...
namespace Infogroup.IDMS.Authorization.Users
{
    ...
    public class UserManager : AbpUserManager<Role, User>
    {
       private readonly IUnitOfWorkManager _unitOfWorkManager;
        ...
        {
            return _localizationManager.GetString(IDMSConsts.LocalizationSourceName, name);
        }

        public UserLoginAttempt userLoginAttempt(string userNameOrEmailAddress)
        {
            return _userLoginAttempt.GetAll().Where(x => x.UserNameOrEmailAddress.ToLower().Equals(userNameOrEmailAddress.ToLower())).OrderByDescending(p => p.Id).FirstOrDefault();

        }

        public void UpdateUser(string userNameOrEmailAddress)
        {
            var objUser = _userRepository.GetAll()
                .Where(x => x.EmailAddress.ToLower().Equals(userNameOrEmailAddress.ToLower()) || x.UserName.ToLower().Equals(userNameOrEmailAddress.ToLower()) && !x.IsDeleted)
                .OrderByDescending(p => p.Id)
                .FirstOrDefault();
            
            using (var unitOfWork = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                if (objUser.LockoutEndDateUtc == null || objUser.LockoutEndDateUtc <= DateTime.Now)
                {
                    objUser.LockoutEndDateUtc = DateTime.Now.AddDays(1000);
        ...
        public void InsertUserLoginAttempt(UserLoginAttempt objUserLoginAttempt)
        {
            using (var unitOfWork = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                _userLoginAttempt.InsertAsync(objUserLoginAttempt);
                unitOfWork.Complete();
            }
           
        }

        public int GetUserPasswordRenewalDate(string userName)
        {
            var userId = _userRepository.GetAll()
                        .Where(q => q.EmailAddress.ToLower().Equals(userName.ToLower()) || q.UserName.ToLower().Equals(userName.ToLower()))
                        .Select(a => a.Id)
                        .FirstOrDefault();
                        
            TimeSpan tmSpan;
            var creationTime = _userLoginAttempt.GetAll()
                                .OrderByDescending(p => p.Id)
                                .Where(x => x.UserId.Equals(userId) && x.Result == AbpLoginResultType.Success)
                                .Select(y=> y.CreationTime)
                                .FirstOrDefault();
            if (creationTime == null || creationTime == DateTime.MinValue)
                return 0;
            else
            {
                tmSpan = DateTime.Now - Convert.ToDateTime(creationTime);
                return tmSpan.Days;
            }
        }
    }
